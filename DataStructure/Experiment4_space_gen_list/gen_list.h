...
protected:
	...
	// 辅助函数模板:
	void ShowList(GenListNode<ElemType> *hd) const;
	// 显示以hd为头结点的广义表
	int CalculateDepth(const GenListNode<ElemType> *hd); // 计算以hd为表头的广义表的深度
...
template <class ElemType>
void GenList<ElemType>::ShowList(GenListNode<ElemType> *hd) const
// 操作结果：显示以hd为头结点的广义表
{
	bool first = true;
	cout << "("; // 广义表以(开始
	for (GenListNode<ElemType> *tmpPtr = hd->nextLink; tmpPtr != nullptr; tmpPtr = tmpPtr->nextLink)
	{ // 依次处理广义表各元素
		if (first)
			first = false; // 第一个元素
		else
...
// 操作结果：显示广义表
{
	ShowList(head); // 调用辅助函数显示广义表
}

template <class ElemType>
int GenList<ElemType>::CalculateDepth(const GenListNode<ElemType> *hd)
// 操作结果：返回以hd为表头的广义表的深度
{
	...
	{ // 求子表的最大深度
		if (tmpPtr->tag == LIST)
		{												       // 子表
			int curSubDepth = CalculateDepth(tmpPtr->subLink); // 子表深度
...
// 操作结果: 将以sourceHead为头结点的广义表复制成以destHead为头结点的广义表
{
	...
	for (GenListNode<ElemType> *tmpPtr = sourceHead->nextLink; tmpPtr != nullptr;
